  ...
  return (
    <>
      ...
      <div className="w-full bg-[#1a1a1a] hidden lg:flex items-center justify-end gap-6 px-6 py-1.5 text-[11px] font-sans tracking-wide fixed top-0 left-0 right-0 z-30">
        <span className="text-[#666]">Mon–Sat · 10am–7pm</span>
        <span className="text-[#C0A15C]">&#9742; +91 9810890353</span>
      </div>

      {/* Main header */}
      <header className="w-full bg-white border-b border-[rgba(192,161,92,0.2)] shadow-sm fixed left-0 right-0 z-30 top-0 lg:top-[29px]">
        <div className="max-w-7xl mx-auto px-4 lg:px-6 flex items-center gap-6 h-[70px]">
          ...
          {/* Gold divider */}
          <div className="hidden xl:block self-stretch w-px bg-[rgba(192,161,92,0.25)] my-3" />
          ...
          <nav className="hidden xl:flex flex-col flex-1 justify-center font-sans">
              ...
              {row1.map((item) => (
                <Link
                  ...
                  className={`relative text-[11.5px] px-2.5 py-1.5 rounded whitespace-nowrap tracking-wide transition-colors
                    ${isActive(item.href)
                      ? "text-[#C0A15C] font-semibold after:absolute after:bottom-0.5 after:left-2.5 after:right-2.5 after:h-[1.5px] after:bg-[#C0A15C] after:rounded-full"
                      : "text-[#555] hover:text-[#C0A15C] hover:bg-[rgba(192,161,92,0.06)]"
                    }`}
                >
                  {item.label}
                </Link>
            ...
            <div className="flex items-center border-t border-dashed border-[rgba(192,161,92,0.18)]">
              {row2.map((item) => (
                <Link
                  key={item.href}
                  href={item.href}
          ...
          <Link
            href="/contact-us"
            className="hidden xl:inline-flex flex-shrink-0 items-center bg-[#C0A15C] hover:bg-[#A2854A] text-white text-[11px] font-semibold uppercase tracking-widest px-5 py-2.5 rounded transition-colors"
          >
            Enquire Now
          </Link>
      ...
      >
        <div className="flex items-center justify-between px-5 py-4 border-b border-[rgba(192,161,92,0.2)]">
          <Image src={logo150} alt="NorthWind Estates" style={{ width: "90px", height: "auto" }} sizes="90px" />
        ...
        <nav className="flex-1 overflow-y-auto flex flex-col font-sans">
          {siteNavLinks.map((item) => (
            <Link
              ...
              className={`px-6 py-3.5 border-b border-[rgba(192,161,92,0.1)] text-sm tracking-wide transition-colors ${
                isActive(item.href) ? "text-[#C0A15C] font-semibold bg-[rgba(192,161,92,0.05)]" : "text-[#444] hover:text-[#C0A15C]"
              }`}
            >
              {item.label}
            </Link>
        ...
        <div className="p-5 border-t border-[rgba(192,161,92,0.2)]">
          <Link
            href="/contact-us"
            onClick={() => setOpen(false)}
            className="flex justify-center py-3 bg-[#C0A15C] hover:bg-[#A2854A] text-white rounded text-sm font-semibold uppercase tracking-widest transition-colors"
          >
            Enquire Now
          </Link>
        </div>
